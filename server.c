...
//function to remove unneccesarry lines from inputs
void
trim_newline (char *text)
{
    ...
    if (text[len] == '\n') //new line
      {
          text[len] = '\0'; //end string
      }
}


//initializing and defining server
void
initialize_server (connection_info * server_info, int port)
{
    if ((server_info->socket = socket (AF_INET, SOCK_STREAM, 0)) < 0) //specifying ipv4 add, sock stream indicates tcp, sock dgram indicates udp
      {
...
//if val is <3
    ...
    if (setsockopt (server_info->socket, SOL_SOCKET, SO_REUSEADDR, (void *) &optVal, optLen) < 0) //SO_REUSEADDR allows binding to prt, if its free and usable,,   &optVal: keeps port alive once connected,,, optLen: gets size of data recieving
      {
...
send_private_message (connection_info clients[], int sender, char *username, char *message_text)
{
    ...
    int i;
	//if client and/or sender are not in the db
    for (i = 0; i < MAX_CLIENTS; i++)
      {
...
//handling all client messages for connecting and disconnecting
void
